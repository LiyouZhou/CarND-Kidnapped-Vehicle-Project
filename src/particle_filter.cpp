...
void ParticleFilter::prediction(double delta_t, double std_pos[],
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    if (abs(yaw_rate) < 0.001) { //  if there is no change in yaw
      ...
      p.x += velocity * (sin(p.theta + delta_t * yaw_rate) - sin(p.theta)) /
             yaw_rate;
      p.y += velocity * (cos(p.theta) - cos(p.theta + delta_t * yaw_rate)) /
             yaw_rate;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    for (auto it = observations.begin(); it != observations.end(); it++) {
      ...
      obs_in_map_coords.x =
          p.x + (cos(p.theta) * it->x) - (sin(p.theta) * it->y);
      obs_in_map_coords.y =
          p.y + (sin(p.theta) * it->x) + (cos(p.theta) * it->y);
    ...
    for (int j = 0; j < map_landmarks.landmark_list.size(); j++) {
      ...
      double dist =
          sqrt(pow(p.x - landmark.x_f, 2) + pow(p.y - landmark.y_f, 2));
    ...
    for (int j = 0; j < observations_in_map_coords.size(); j++) {
      ...
      double exponent = (pow((obs.x - feature.x_f) / std_landmark[0], 2) +
                         pow((obs.y - feature.y_f) / std_landmark[1], 2)) /
                        2;
...
void ParticleFilter::resample() {
  ...
  // weight. construct a vector of weights
